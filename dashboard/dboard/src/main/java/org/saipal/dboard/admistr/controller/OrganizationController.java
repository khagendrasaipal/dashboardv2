package org.saipal.dboard.admistr.controller;

import java.util.Map;

import javax.servlet.http.HttpServletRequest;

import org.saipal.dboard.admistr.model.Organization;
import org.saipal.dboard.admistr.service.OrganizationService;
import org.saipal.fmisutil.util.Messenger;
import org.saipal.fmisutil.util.ValidationService;
import org.saipal.fmisutil.util.Validator;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.DeleteMapping;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.PutMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/organization")
public class OrganizationController {
	@Autowired
	OrganizationService objService;
	@Autowired
	ValidationService validationService;

	@GetMapping("")
	public ResponseEntity<Map<String, Object>> index(HttpServletRequest request) {
		return objService.index();
	}
	
	@GetMapping("/getlistOpen")
	public ResponseEntity<Map<String, Object>> getlistOpen(HttpServletRequest request) {
		return objService.index1();
	}

	@PostMapping("")
	public ResponseEntity<Map<String, Object>> store(HttpServletRequest request) {

		Validator validator = validationService.validate(Organization.rules());
		if (validator.isFailed()) {
			return Messenger.getMessenger().setData(validator.getErrorMessages()).error();
		} else {
			return objService.store();
		}
	}
	
	@GetMapping("/get-adminlvl")
	public ResponseEntity<Map<String, Object>> getAdmLvl(HttpServletRequest request) {
		return objService.getAdminlvl();
	}
	@GetMapping("/get-federal")
	public ResponseEntity<Map<String, Object>> getFederal(HttpServletRequest request) {
		return objService.getFederal();
	}
	@GetMapping("/get-parent/{id}")
	public ResponseEntity<Map<String, Object>> getParent(HttpServletRequest request, @PathVariable String id) {
		return objService.getParent(id);
	}
	@GetMapping("/get-parent-orgs")
	public ResponseEntity<Map<String, Object>> getParentOrgs(HttpServletRequest request) {
		return objService.getParentOrgs();
	}
	
	@GetMapping("/get-orglevel")
	public ResponseEntity<Map<String, Object>> getOrglevel(HttpServletRequest request) {
		return objService.getOrglevel();
	}
	
	@GetMapping("/get-admin/{id}")
	public ResponseEntity<Map<String, Object>> getAdmin(HttpServletRequest request, @PathVariable String id) {
		return objService.getAdmin(id);
	}
	

	@GetMapping("/{id}")
	public ResponseEntity<Map<String, Object>> edit(HttpServletRequest request, @PathVariable String id) {
		return objService.edit(id);
	}

	@PutMapping("/{id}")
	public ResponseEntity<Map<String, Object>> update(HttpServletRequest request, @PathVariable String id) {
		Validator validator = validationService.validate(Organization.rules());
		if (validator.isFailed()) {
			return Messenger.getMessenger().setData(validator.getErrorMessages()).error();
		} else {
			return objService.update(id);
		}
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<Map<String, Object>> destroy(HttpServletRequest request, @PathVariable String id) {
		
		return objService.destroy(id);
	}
	@GetMapping("/getorgs/session")
	public ResponseEntity<Map<String, Object>> getsession(HttpServletRequest request){
		return objService.getOrgs();
	}
	
	@GetMapping("/getorganization")
	public ResponseEntity<Map<String, Object>> getorganization(HttpServletRequest request){
		return objService.getorganization();
	}
	
	@GetMapping("/getFyrange")
	public ResponseEntity<Map<String, Object>> getFyrange(HttpServletRequest request){
		return objService.getFyrange();
	}
	
	@GetMapping("/getorgs")
	public ResponseEntity<Map<String, Object>> getorgs(HttpServletRequest request){
		return objService.getorg();
	}

}
