package org.saipal.dboard.setup.publicDashboardSetup;
...
//		System.out.println("Create ");
		...
		sql = "INSERT INTO public_dashboard_setup (p_id,fy,indicator,status,orgid,created_by,chart_type,time) VALUES (?,?,?,?,?,?,?,?)";
		DbResponse rowEffect = db.execute(sql,
				Arrays.asList(model.p_id, model.fy, model.indicator,model.status,session("orgid"),session("empid"), model.chart_type,model.time));
	...
	public ResponseEntity<Map<String, Object>> edit(String id) {
		...
		String sql = "select id,indicator,fy,p_id,status,chart_type,time from "
				+ table + " where id=?";
	...
	public ResponseEntity<Map<String, Object>> update(String id) {
		...
		String sql = "UPDATE public_dashboard_setup set status=?,indicator=?,fy=?,p_id=?,orgid=?,created_by=?, chart_type=?,time=? where id=?";
		rowEffect = db.execute(sql,
				Arrays.asList(model.status,model.indicator, model.fy,model.p_id,session("orgid"),session("empid"),model.chart_type,model.time,id));
