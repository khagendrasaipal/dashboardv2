...
export class DboardComponent implements OnInit,AfterViewInit {
  ...
  constructor(private RS: DboardService, private toastr: ToastrService, private fb: FormBuilder,private loginService: LoginService,private modalService: BsModalService) { 
    ...
    this.srchForm = new FormGroup({
      ...
      ldate: new FormControl((new Date()).toISOString().substring(0,10)),
      
    });

    this.logForm = new FormGroup({
      logdate: new FormControl((new Date()).toISOString().substring(0,10)),
  ...
  ngOnInit(): void {
    this.ldate=(new Date()).toISOString().substring(0,10);
    ...
    if(this.usertype=="hmis"||this.usertype=="province"){
      $("#todaylog").show();
    }else{
      $("#todaylog").hide();
    }
    ...
    this.getList();
    this.gettodaylog((new Date()).toISOString().substring(0,10));
    this.getOrgs();
    this.getDefault();
    this.getProgram();
    this.getSectorData();
  ...
  getOrgs(){
    this.RS.getorganization().subscribe({
      next: (result:any) => {
...
fyRng:any;
  getFyrange(){
    this.RS.getFyrange().subscribe({
      next: (result:any) => {
          // console.log(result.data[0]);
          this.fys=result.data[0];
          this.fyRng = this.fyRange(this.fys.from,this.fys.to);
          console.log(this.fys.from);
       },
       error : err => {
        //  this.toastr.error(err.error, 'Error');
       }
...
fyRange(from:any,to:any){
  const fys = [];
  // let from=this.fys.from;
  // let to=this.fys.to;
  ...
  gettodaylog(date:any){
    this.usertype=localStorage.getItem("usertype");
    this.RS.gettodayLog( this.usertype,date).subscribe({
      next: (result:any) => {
...
createFItem(id:any){
        ...
        next:(result:any) => {
          if(this.cForm.value.charttype=="table"){
            ...
            exporting: {
              ...
              buttons: {
                contextButton: {
                    menuItems: ['downloadPNG', 'downloadJPEG', 'downloadPDF', 'downloadSVG'] // Check if these options are set correctly
                }
            }
            },
            plotOptions: {
  ...
  getList(pageno?: number | undefined) {
    const page = pageno || 1;
    this.RS.getList(this.ldate,this.pagination.perPage, page, this.searchTerm, this.column, this.isDesc).subscribe({
      next:(result: any) => {
  ...
  search() {
    ...
    this.ldate=this.srchForm.value.ldate;
    this.getList();
